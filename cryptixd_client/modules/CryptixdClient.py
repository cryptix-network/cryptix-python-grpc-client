...
class CryptixdClient(object):
    def __init__(self, cryptixd_host: str, cryptixd_port: int = 16110):
        ...
        self.cryptix_stream = CryptixdStream(self.cryptixd_host, self.cryptixd_port)
        self.__current_id = 1
    ...
    async def request(
        ...
        await self.cryptix_stream.send(command, params, msg_id)

        if wait_for_response:
            return await self.cryptix_stream.read(msg_id)
    ...
    async def get_current_network(self):
        """
        GetCurrentNetworkRequestMessage requests the network cryptixd_client is currently running against.
    ...
    async def get_peer_addresses(self):
        """
        GetPeerAddressesRequestMessage requests the list of known cryptixd_client addresses in the
    ...
    async def get_connected_peer_info(self):
        ...
        currently connected to this cryptixd_client.
        :return:
        """
        return await self.request(
    ...
    async def add_peer(self, address: str, is_permanent: bool):
        """
        AddPeerRequestMessage adds a peer to cryptixd_client's outgoing connection list.
        This will, in most cases, result in cryptixd_client connecting to said peer.
        :param address:
        :param is_permanent:
        :return:
        """
    ...
    async def get_virtual_chain_from_block(
        ...
        parent chain from some startHash to this cryptixd_client's current virtual
        :param start_hash:
    ...
    async def get_blocks(
        ...
        cryptixd_client's current virtual.
        :param low_hash:
    ...
    async def get_block_count(self):
        """
        GetBlockCountRequestMessage requests the current number of blocks in this cryptixd_client.
    ...
    async def get_utxos_by_addresses(self, addresses: list[str]):
        """
        GetUtxosByAddressesRequestMessage requests all current UTXOs for the given cryptixd_client addresses

        This call is only available when this cryptixd_client was started with `--utxoindex`
        :param addresses:
        :return:
        """
        return await self.request(
    ...
    def notify_block_added(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback("blockAddedNotification", f)
        )
    ...
    def notify_virtual_chain_changed(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback("virtualChainChangedNotification", f)
        )
        asyncio.get_running_loop().create_task(
    ...
    def notify_finality_conflict(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback("finalityConflictNotification", f)
        )
        asyncio.get_running_loop().create_task(
    ...
    def notify_sink_blue_score_changed(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback("sinkBlueScoreChangedNotification", f)
        )
        asyncio.get_running_loop().create_task(
    ...
    def notify_virtual_daa_score_changed(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback("virtualDaaScoreChangedNotification", f)
        )
        asyncio.get_running_loop().create_task(
    ...
    def notify_pruning_point_utxo_set_override(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback(
    ...
    def notify_new_block_template(self, f):
        asyncio.get_running_loop().create_task(
            self.cryptix_stream.register_callback("newBlockTemplateNotification", f)
        )
        asyncio.get_running_loop().create_task(
